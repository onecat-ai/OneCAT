...
class Linear(nn.Module, LoraLayer):
    ...
    def forward(self, x: torch.Tensor, multi_scale_ge_lora_mask=None, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        self.training = True
        if self.training:
            multi_scale_ge_lora_mask = torch.stack(multi_scale_ge_lora_mask).permute(1,2,0).repeat_interleave(self.rank,dim=-1)*1
            for active_adapter in self.lora_A.keys():
                ...
                result = result + lora_B(multi_scale_ge_lora_mask*lora_A(dropout(x))) * scaling * self.lora_num
            result = result.to(torch_result_dtype)
        else:
            multi_scale_ge_lora_mask = torch.stack(multi_scale_ge_lora_mask).permute(1,2,0)*1
            nonzero_indices = torch.nonzero(multi_scale_ge_lora_mask[0][0])
            start_idx = nonzero_indices.squeeze() 
            for active_adapter in self.lora_A.keys():
                lora_A = self.lora_A[active_adapter]
                lora_B = self.lora_B[active_adapter]
                lora_A_k = lora_A.weight[start_idx*self.rank:start_idx*self.rank+self.rank, :]  # [64, 1536]
                lora_B_k = lora_B.weight[:, start_idx*self.rank:start_idx*self.rank+self.rank]  # [8960, 64]
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                x = x.to(lora_A.weight.dtype)

                lora_output = dropout(x) @ lora_A_k.T  # [batch_size, seq_len, 64]
                lora_output = lora_output @ lora_B_k.T  # [batch_size, seq_len, 8960]
                result = result + lora_output * scaling * self.lora_num
            result = result.to(torch_result_dtype)
