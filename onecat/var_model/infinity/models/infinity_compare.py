...
import onecat.var_model.infinity.utils.dist as dist
from onecat.var_model.infinity.utils.dist import for_visualize
from onecat.var_model.infinity.models.basic import flash_fused_op_installed, AdaLNBeforeHead, CrossAttnBlock, SelfAttnBlock, CrossAttention, FastRMSNorm, precompute_rope2d_freqs_grid
from onecat.var_model.infinity.utils import misc
from onecat.var_model.infinity.models.flex_attn import FlexAttn
from onecat.var_model.infinity.utils.dynamic_resolution import dynamic_resolution_h_w, h_div_w_templates

try:
    from onecat.var_model.infinity.models.fused_op import fused_ada_layer_norm, fused_ada_rms_norm
except:
...
class Infinity(nn.Module):
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        sos = kv_compact.unsqueeze(1).expand(B, 1, -1) 
        x_BLC = torch.cat((sos, self.word_embed(x_BLC_wo_prefix)), dim=1)
        x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule)


        return  x_BLC, scale_schedule
